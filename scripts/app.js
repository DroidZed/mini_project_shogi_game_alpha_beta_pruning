class ShogiGame {
  ...
  getPossibleMoves(row, col) {
    ...
    switch (piece.type) {
      case "king":
        ...
        break;
      case "rook":
        moves = piece.promoted
          ? this.getDragonMoves(row, col)
          : this.getRookMoves(row, col);
        break;
      case "bishop":
        moves = piece.promoted
          ? this.getHorseMoves(row, col)
      ...
      case "silver":
        moves = piece.promoted
          ? this.getGoldMoves(row, col, direction)
          : this.getSilverMoves(row, col, direction);
        break;
      case "knight":
        moves = piece.promoted
          ? this.getGoldMoves(row, col, direction)
          : this.getKnightMoves(row, col, direction);
        break;
      case "lance":
        moves = piece.promoted
          ? this.getGoldMoves(row, col, direction)
          : this.getLanceMoves(row, col, direction);
        break;
      case "pawn":
        moves = piece.promoted
          ? this.getGoldMoves(row, col, direction)
          : this.getPawnMoves(row, col, direction);
        break;
    }

    return moves.filter((move) =>
      this.isValidMove(row, col, move.row, move.col)
    );
  }
  ...
  getGoldMoves(row, col, direction) {
    ...
    const goldDirections = [
      [-1, -1],
      [-1, 0],
      [-1, 1], // Forward diagonals and straight
      [0, -1],
      [0, 1], // Sideways
      [1, 0], // Backward
    ];
  ...
  isValidMoveForPiece(fromRow, fromCol, toRow, toCol) {
    ...
    if (!isPossible) {
      this.logEvent(
        `Move from ${fromRow},${fromCol} to ${toRow},${toCol} not in possible moves.`
      );
      return false;
    }
    ...
    if (inCheck) {
      this.logEvent(
        `Move from ${fromRow},${fromCol} to ${toRow},${toCol} rejected: leaves king in check.`
      );
    }
      ...
      for (let col = 0; col < 9; col++) {
        ...
        if (piece && piece.owner === opponent) {
          ...
          if (
            ...
            this.logEvent(
              `${player}'s king at ${kingPos.row},${kingPos.col} is in check by piece at ${row},${col}.`
            );
            return true;
          }
        }
  ...
  createBoardSquares() {
    ...
    for (let row = 0; row < 9; row++) {
      for (let col = 0; col < 9; col++) {
        ...
        square.className =
          "board-square w-12 h-12 bg-amber-50 hover:bg-amber-100 flex items-center justify-center cursor-pointer transition-colors duration-200";
        square.dataset.row = row;
        square.dataset.col = col;
        const piece = document.createElement("div");
        piece.className =
          "shogi-piece select-none w-full h-full flex items-center justify-center";
    ...
    document.addEventListener("dragend", (e) => {
      ...
      if (this.dragState.originalSquare) {
        const originalSquare = this.boardElement.querySelector(
          `[data-row="${this.dragState.originalSquare.row}"][data-col="${this.dragState.originalSquare.col}"]`
        );
      ...
      if (this.dragState.originalSquare) {
        if (
          this.isValidMoveForPiece(
            this.dragState.originalSquare.row,
            this.dragState.originalSquare.col,
            toRow,
            toCol
          )
        ) {
        ...
        if (
          this.canDropPiece(
            this.dragState.droppingPiece.type,
            this.dragState.droppingPiece.owner,
            toRow,
            toCol
          )
        ) {
  ...
  clearHighlights() {
    this.boardElement
      .querySelectorAll(
        ".board-square.selected, .board-square.highlighted, .board-square.drop-zone"
      )
      .forEach((sq) => {
  ...
  makeMove(fromRow, fromCol, toRow, toCol) {
    ...
    if (capturedPiece) {
      this.capturedPieces[moveMaker].push({
  ...
  canDropPiece(pieceType, owner, row, col) {
    ...
    if (inCheck) {
      this.logEvent(
        `Drop of ${pieceType} at ${row},${col} rejected: leaves king in check.`
      );
      return false;
    }
  ...
  makeAIMove() {
    ...
    if (bestMove) {
      if (bestMove.type === "drop") {
        document.getElementById("best-move").textContent = `${this.getPieceChar(
          { type: bestMove.drop.type }
        )}*${9 - bestMove.to.col}${String.fromCharCode(97 + bestMove.to.row)}`;
        this.logEvent(
          `AI dropping ${bestMove.drop.type} at ${bestMove.to.row},${bestMove.to.col}`
        );
        this.dropPiece(
          bestMove.drop.type,
          "gote",
        ...
        document.getElementById("best-move").textContent = this.getMoveNotation(
          bestMove.from.row,
          bestMove.from.col,
          bestMove.to.row,
          bestMove.to.col,
        ...
        this.logEvent(
          `AI moving ${piece.type} from ${bestMove.from.row},${bestMove.from.col} to ${bestMove.to.row},${bestMove.to.col}`
        );
        this.makeMove(
          bestMove.from.row,
          bestMove.from.col,
  ...
  getAllMovesForPlayer(player) {
    ...
    const uniqueCaptured = [
      ...new Map(this.capturedPieces[player].map((p) => [p.type, p])).values(),
    ];
      ...
      for (let r = 0; r < 9; r++) {
        for (let c = 0; c < 9; c++) {
          if (this.canDropPiece(captured.type, player, r, c)) {
            moves.push({
              drop: { type: captured.type, owner: player },
              to: { row: r, col: c },
              type: "drop",
            });
          }
  ...
  applyMove(move) {
    ...
    if (move.type === "move") {
      ...
      const index = this.capturedPieces[pieceToDrop.owner].findIndex(
        (p) => p.type === pieceToDrop.type
      );
      if (index > -1) {
  ...
  undoMove(undoData) {
    ...
    if (move.type === "move") {
      ...
      this.capturedPieces[move.drop.owner].push({
        type: undoData.droppedPieceType,
        promoted: false,
      });
    }
  }
      ...
      for (let col = 0; col < 9; col++) {
        ...
        if (piece) {
          let pieceValue = piece.promoted
            ? promotedValues[piece.type] || pieceValues[piece.type]
  ...
  updateGameStatistics() {
    const totalNodes =
      this.gameStats.nodesEvaluated + this.gameStats.nodesPruned;
    const pruningRate =
      totalNodes > 0 ? (this.gameStats.nodesPruned / totalNodes) * 100 : 0;

    document.getElementById("nodes-evaluated").textContent =
      this.gameStats.nodesEvaluated.toLocaleString();
    document.getElementById("nodes-pruned").textContent =
      this.gameStats.nodesPruned.toLocaleString();
    document.getElementById("time-taken").textContent = Math.round(
      this.gameStats.timeTaken
    ).toLocaleString();
    document.getElementById(
      "pruning-rate"
    ).textContent = `${pruningRate.toFixed(1)}%`;
  ...
  updateCapturedPieces() {
    this.renderCapturedPieces(
      document.getElementById("player-captured"),
      this.capturedPieces.sente,
      "sente"
    );
    this.renderCapturedPieces(
      document.getElementById("ai-captured"),
      this.capturedPieces.gote,
      "gote"
    );
  }
  ...
  updateMoveHistory() {
    ...
    [...this.moveHistory].reverse().forEach((move, index) => {
      ...
      moveElement.className =
        "move-entry text-xs py-1 px-2 hover:bg-gray-600/50 rounded flex justify-between";
  ...
  startNewGame() {
    ...
    document.getElementById("game-log").innerHTML =
      '<div class="text-gray-500 text-sm text-center py-8">Game log is empty</div>';
  ...
  isCheckmate(player) {
    ...
    if (this.getPossibleMovesForPlayer(player).length > 0) {
      this.logEvent(
        `${player} is in check, but has legal moves. Not checkmate.`
      );
      return false;
    }
    ...
    for (const captured of this.capturedPieces[player]) {
      for (let r = 0; r < 9; r++) {
        for (let c = 0; c < 9; c++) {
          if (this.canDropPiece(captured.type, player, r, c)) {
            this.logEvent(
              `${player} has an escape drop: ${captured.type} at ${r},${c}`
            );
            return false;
          }
        }
    ...
    this.logEvent(
      `Checkmate detected for ${player}. No legal moves or drops available.`
    );
